...
void fs_log_store::write_at(ulong index, ptr<log_entry>& entry) {
    ...
    ptr<buffer> ibuf(buffer::alloc(sz_ulong));
    ibuf->put(data_pos);
    ibuf->pos(0);
    idx_file_ << *ibuf;
...
ptr<std::vector<ptr<log_entry>>> fs_log_store::log_entries(ulong start, ulong end) {
    ...
    if (start < buffer_first_idx) {
        ...
        for (int i = 0; i < (int)(lend - lstart); ++i) {
            ...
            ptr<buffer> entry_buf(buffer::alloc(data_sz));
            data_file_ >> *entry_buf;
...
ptr<log_entry> fs_log_store::entry_at(ulong index) {
...

    {
        ...
        ptr<buffer> entry_buf(buffer::alloc((int)(end_d_pos - d_pos)));
        data_file_ >> *entry_buf;
...
ulong fs_log_store::term_at(ulong index) {
...

    {
        ...
        ptr<buffer> term_buf(buffer::alloc(sz_ulong));
        data_file_ >> *term_buf;
...
ptr<buffer> fs_log_store::pack(ulong index, int32 cnt) {
    ...
    ptr<buffer> idx_buf(buffer::alloc(static_cast<int>(end_offset - offset) * sz_ulong));
    idx_file_ >> *idx_buf;
    ulong end_of_data(0);
    if (read_to_end) {
        ...
        ptr<buffer> end_d_idx_buf(buffer::alloc(sz_ulong));
    ...
    ptr<buffer> data_buf(buffer::alloc(static_cast<int>(end_of_data - start_of_data)));
    data_file_ >> *data_buf;
...
void fs_log_store::apply_pack(ulong index, buffer& pack) {
    ...
    if (local_idx == entries_in_store_) {
        ...
        idx_file_.seekg(idx_pos);
        ptr<buffer> data_pos_buf(buffer::alloc(sz_ulong));
...
bool fs_log_store::compact(ulong last_log_index) {
    ...
    do {
        if (last_log_index >= start_idx_ + entries_in_store_ - 1) {
            ...
            ptr<buffer> start_idx_buf(buffer::alloc(sz_ulong));
...
void fs_log_store::fill_buffer() {
    ...
    if (idx_file_.tellg() > 0) {
        ...
        idx_file_.seekg(idx_pos);
        ptr<buffer> idx_buf(buffer::alloc(static_cast<size_t>(idx_file_len - idx_pos)));
        idx_file_ >> *idx_buf;
